...
function simpletheme_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'simpletheme' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'simpletheme_custom_background_args',
			array(
...
function simpletheme_scripts() {
	wp_enqueue_style( 'simpletheme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'simpletheme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'simpletheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
